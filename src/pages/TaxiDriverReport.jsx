...
export default function TaxiDriverReport() {
    ...
    function handleLogout() {
        localStorage.removeItem("taxi_driver_session");
        showToast("Đã đăng xuất!", "success");

        setTimeout(() => {
            window.location.href = "/driver-login-taxi";
        }, 700);
    }
    ...
    async function loadDriverAndLastReport(driverTaxiId) {
        ...
        if (lastReport && !errLast) {
            ...
            beStart = Number(driver.be_baseline || 0);
            smStart = Number(driver.sm_baseline || 0);
        }
    ...
    return (
        <div className="min-h-screen bg-[#0b111f] text-slate-100 p-4">
            ...
            <div className="max-w-xl mx-auto bg-slate-900/70 border border-slate-700 rounded-xl p-5 relative">
                ...
                {driverTaxi && (
                    <div className="bg-slate-800/70 border border-slate-600 rounded-lg p-3 mb-5 text-sm">
                        <div><b>Tài xế:</b> {driverTaxi.full_name}</div>
                        <div><b>SĐT:</b> {driverTaxi.phone}</div>
                        <div><b>Biển số:</b> {driverTaxi.car_plate || "—"}</div>
                        <div><b>Tỉ lệ tài xế:</b> {driverTaxi.driver_share}%</div>
                    </div>
                )}
                ...
                <div className="space-y-4 text-sm">
                    ...
                    {/* VÍ BE */}
                    <WalletBox
                        title="Ví BE trong app"
                        start={form.be_wallet_start}
                        end={form.be_wallet_end}
                        onChangeEnd={(v) => setForm({ ...form, be_wallet_end: v })}
                    />

                    {/* VÍ SM */}
                    <WalletBox
                        title="Ví Xanh SM trong app"
                        start={form.sm_wallet_start}
                        end={form.sm_wallet_end}
                        onChangeEnd={(v) => setForm({ ...form, sm_wallet_end: v })}
                    />
...
function Input({ label, type = "number", value, onChange, readOnly }) {
    return (
        <div>
            ...
            <input
                type={type}
                className={`w-full bg-slate-700 px-3 py-2 rounded text-sm ${readOnly ? "opacity-80 cursor-not-allowed" : ""
                    }`}
                value={value}
                readOnly={readOnly}
...
function WalletBox({ title, start, end, onChangeEnd }) {
    return (
        <div className="border border-slate-700 rounded-lg p-3">
            <div className="font-semibold mb-2 text-amber-300">{title}</div>
            <div className="grid grid-cols-2 gap-3">
                <Input label="Ví đầu ngày" value={start} readOnly />
                <Input
                    label="Ví cuối ngày"
                    value={end}
